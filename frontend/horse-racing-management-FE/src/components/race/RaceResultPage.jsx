  ...
  return (
    <div className="rr-podium">
      {order.map((idx, col) => {
        ...
        const pos = r.rank;
        return (
          <div key={r.id} className={`rr-podium__slot rr-podium__slot--pos${pos}`}>
            ...
            {jockeyUser && <div className="rr-podium__jockey">{jockeyUser.fullName || jockeyUser.username}</div>}
            {r.rewards > 0 && <div className="rr-podium__prize">{formatPrize(r.rewards)}</div>}
          </div>
        );
      })}
    </div>
  ...
  return (
    <div className="rr-table-wrap">
      <table className="rr-table">
        <thead>
          <tr><th>Rank</th><th>Horse</th><th>Jockey</th><th>Lane</th><th>Reward</th></tr>
        </thead>
        <tbody>
          {sorted.map((r) => {
            const horse = r.raceHorse?.horse;
            const jockeyUser = r.raceHorse?.jockey?.user;
            return (
              <tr key={r.id} className={`rr-table__row${r.rank <= 3 ? ` rr-table__row--top${r.rank}` : ''}`}>
                <td>
                ...
                <td className="rr-table__jockey">{jockeyUser?.fullName || jockeyUser?.username || '—'}</td>
                <td className="rr-table__lane">{r.raceHorse?.laneNumber ?? '—'}</td>
                <td className="rr-table__reward">
                  {r.rewards > 0 ? formatPrize(r.rewards) : <span className="rr-table__no-reward">—</span>}
                </td>
              </tr>
            );
          })}
...
function OtherRaceCard({ race, onClick, isActive }) {
  return (
    <button type="button" className={`rr-bar__card${isActive ? ' rr-bar__card--active' : ''}`} onClick={onClick}>
    ...
    <div className="rr-page">
      ...
      <header className="rr-hero">
        ...
        <div className="rr-hero__content">
          ...
          <div className="rr-hero__chips">
            ...
            {race?.totalprizepool && <span className="rr-chip rr-chip--prize">{formatPrize(race.totalprizepool)}</span>}
          </div>
          <Podium top3={top3} />
        </div>
      </header>
      ...
      <section className="rr-standings">
        <div className="rr-standings__inner">
          ...
          {results.length === 0
            ? <div className="rr-empty"><p>No results have been recorded for this race yet.</p></div>
