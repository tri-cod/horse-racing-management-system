...
function RaceMiniCard({ race, featured = false }) {
  const dateStr = race.startTime ? new Date(race.startTime).toISOString().slice(0, 10) : null;
  const to = dateStr ? `/races?date=${dateStr}` : '/races';
  return (
    <Link to={to} className={`nav-race-card${featured ? ' nav-race-card--featured' : ''}`}>
      {race.bannerImageurl && (
        ...
        <Container className="header__utility-inner">
          ...
          <div className="header__utility-links">
            <div className="header__quick-links">
              <Link to="/races"   className={`header__quick-link${pathname.startsWith('/races')   ? ' active' : ''}`}>Schedule</Link>
              <span className="header__util-sep">·</span>
              <Link to="/results" className={`header__quick-link${pathname === '/results'          ? ' active' : ''}`}>Results</Link>
              <span className="header__util-sep">·</span>
              <Link to="/jockeys" className={`header__quick-link${pathname === '/jockeys'          ? ' active' : ''}`}>Jockeys</Link>
              <span className="header__util-sep">·</span>
              <Link to="#" className="header__quick-link">Contact</Link>
              <span className="header__util-sep">·</span>
              <Link to="#" className="header__quick-link">Help</Link>
            </div>
              ...
              <>
                ...
                <div className="header__user">
                  <button type="button" className="header__avatar" title="Account">
                    {user.username || 'User'}
                    <ChevronDown size={12} className="header__avatar-chevron" />
                  </button>
