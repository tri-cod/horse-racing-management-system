...
export default function UsersTable({ users, onChangeRole, onChangeStatus }: UsersTableProps) {
  if (users.length === 0) {
    ...
    <div className="overflow-hidden border border-rim">
      <table className="w-full">
        ...
        <tbody className="divide-y divide-rim">
          {users.map((u) => (
            <UserRow key={u.id} user={u} onChangeRole={onChangeRole} onChangeStatus={onChangeStatus} />
          ))}
        </tbody>
      </table>
    </div>
