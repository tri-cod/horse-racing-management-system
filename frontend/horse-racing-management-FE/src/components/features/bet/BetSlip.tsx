...
import { MIN_BET, isBettable, fmtVnd, type HorseEntry, type Selection } from './betHelpers';
...
export default function BetSlip({
  user, canBet, selectedRace, betHorses, selections, betTotal, maxPayout,
  insufficientFunds, betError, betLoading, onRemove, onClearAll, onSubmit,
}: {
  ...
  if (!isBettable(selectedRace.status)) return (
    <div className="rounded-md border border-rim bg-surface-raised p-5">
      <div className="flex items-start gap-3">
        <AlertCircle size={15} className="mt-0.5 shrink-0 text-ink-4" strokeWidth={1.5} />
        <div>
        ...
        <>
          ...
          <div className="border-t border-rim bg-surface-overlay/60 px-5 py-4 space-y-3">
            ...
            <div className="flex items-center justify-between text-sm">
              ...
              <span className="tnum font-bold text-gold-hi">{fmtVnd(maxPayout)}</span>
            </div>

            {/* Caught before submit so the punter is not told about it by a failed
                request after the fact. */}
            {insufficientFunds && (
              <div className="flex items-start gap-2 rounded bg-warn-subtle px-3 py-2 text-xs text-warn">
                <AlertCircle size={13} className="mt-0.5 shrink-0" />
                <span>
                  This stake is more than your wallet balance.{' '}
                  <Link to="/my-wallet" className="font-semibold underline hover:no-underline">Top up</Link>
                  {' '}or lower the amount.
                </span>
              </div>
            )}
            ...
            <button
              ...
              <Ticket size={14} />
              {betLoading ? 'Processing' : 'Review Bet'}
            </button>
          </div>
        </>
      )}
