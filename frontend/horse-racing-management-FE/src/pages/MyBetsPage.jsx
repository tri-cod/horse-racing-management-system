...
import {
  Ticket, RefreshCw, TrendingUp, DollarSign,
  CheckCircle, ChevronDown, ChevronUp, Calendar,
  Trophy, Target,
} from 'lucide-react';
...
const RESULT_CLS = {
  WON:     'result-pill--won',
  LOST:    'result-pill--lost',
  PENDING: 'result-pill--pending',
};
    ...
    <div className="bet-items-wrap">
      <table className="bet-items-table">
        <thead>
          <tr>
            <th>Horse</th>
            <th className="ta-r">Stake</th>
            <th className="ta-r">Odds</th>
            <th className="ta-r">Payout</th>
            <th>Result</th>
          </tr>
        </thead>
        <tbody>
          {items.map((it) => (
            <tr key={it.id} className="bet-items-table__row">
              ...
              <td className="ta-r bet-items-table__odds">
                {it.odds != null ? `×${parseFloat(it.odds).toFixed(2)}` : '—'}
              </td>
              <td className="ta-r">
                {it.payout != null ? <strong>{fmt(it.payout)}</strong> : '—'}
              </td>
              <td>
                {it.resultStatus ? (
                  <span className={`result-pill ${RESULT_CLS[it.resultStatus] ?? ''}`}>
                    {RESULT_LABEL[it.resultStatus] ?? it.resultStatus}
                  </span>
                ) : (
                  <span className="result-pill result--pending">—</span>
                )}
              </td>
            </tr>
          ))}
  ...
  return (
    <div className={`bet-row${open ? ' bet-row--open' : ''}`}>
      ...
      >
        ...
        <div className="bet-row__col bet-row__col--status">
          <BetStatusBadge status={bet.status} />
        </div>
        <div className="bet-row__col bet-row__col--toggle">
      ...
      {open && (
        <div className="bet-row__detail">
          ...
          <div className="bet-row__detail-footer">
            <Link to={`/races/${bet.raceId}`} className="ui-btn ui-btn--outline ui-btn--sm">
              View race
            </Link>
          </div>
        </div>
...
export default function MyBetsPage() {
  ...
  const totalPayout  = wonBets.reduce(
    (s, b) => s + (b.betItems ?? []).reduce((si, it) => si + (Number(it.payout) || 0), 0), 0
  );
    ...
    <div className="my-bets-page">
      ...
      <section className="my-bets-page__hero">
        <div className="my-bets-page__hero-inner">
          ...
          <p className="my-bets-page__hero-sub">
            Track every wager, review results and monitor your win rate.
          </p>
        </div>
      </section>
      ...
      <div className="my-bets-page__body">
        ...
        <div className="my-bets-page__stats">
          <div className="bstat-card">
            <div className="bstat-card__icon bstat-card__icon--blue"><Ticket size={20} /></div>
            <div className="bstat-card__body">
              <span className="bstat-card__val">{bets.length}</span>
              <span className="bstat-card__lbl">Total bets</span>
            </div>
          </div>
          <div className="bstat-card">
            <div className="bstat-card__icon bstat-card__icon--dark"><DollarSign size={20} /></div>
            <div className="bstat-card__body">
              <span className="bstat-card__val bstat-card__val--sm">{fmt(totalWagered)}</span>
              <span className="bstat-card__lbl">Total wagered</span>
            </div>
          </div>
          <div className="bstat-card">
            <div className="bstat-card__icon bstat-card__icon--green"><CheckCircle size={20} /></div>
            <div className="bstat-card__body">
              <span className="bstat-card__val">{wonBets.length}</span>
              <span className="bstat-card__lbl">Wins</span>
            </div>
          </div>
          <div className="bstat-card">
            <div className="bstat-card__icon bstat-card__icon--gold"><TrendingUp size={20} /></div>
            <div className="bstat-card__body">
              <span className="bstat-card__val bstat-card__val--sm">{fmt(totalPayout)}</span>
              <span className="bstat-card__lbl">Total winnings</span>
            </div>
          </div>
        </div>
        ...
        {!loading && bets.length > 0 && (
          <div className="my-bets-page__winrate">
            <Trophy size={18} />
            <span>
              Your win rate: <strong>{winRate}%</strong>
              {winRate >= 50 ? ' — Great form! 🔥' : ' — Keep going! 💪'}
            </span>
          </div>
        )}
          ...
          <div className="my-bets-page__tabs" role="tablist">
            {TABS.map((tab) => {
              ...
              return (
                ...
                >
                  {tab.label}
                  {cnt > 0 && <span className="my-bets-page__tab-cnt">{cnt}</span>}
                </button>
              );
            })}
          </div>
        ...
        {error && (
          <div className="my-bets-page__error">
            <span>{error}</span>
            <button type="button" onClick={fetchBets}>Retry</button>
          </div>
        )}

        {loading && !error && (
          <div className="my-bets-page__skeletons">
            {[1, 2, 3].map((i) => <div key={i} className="bet-skeleton" />)}
          </div>
        )}
        ...
        {!loading && !error && (
          filtered.length === 0 ? (
            <div className="my-bets-page__empty">
              ...
              <p>
                {activeTab === 'ALL'
                  ? 'Head to a live race and try your luck!'
            ...
            <div className="my-bets-page__list">
              {filtered.map((bet) => <BetRow key={bet.id} bet={bet} />)}
            </div>
          )
        )}
      </div>
