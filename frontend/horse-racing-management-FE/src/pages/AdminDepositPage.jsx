  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (type === 'approve') {
        ...
        addToast(`Approved deposit of ${fmt(deposit.amount)} for ${deposit.user?.username ?? deposit.user?.fullName ?? 'user'}`, 'success');
      } else {
        await rejectDeposit(deposit.id, 'Rejected by admin');
        addToast(`Rejected deposit of ${fmt(deposit.amount)} for ${deposit.user?.username ?? deposit.user?.fullName ?? 'user'}`, 'error');
      }
      closeConfirm();
      fetchDeposits();
    } catch (e) {
  ...
  return (
    <div className="admin-deposit-page">
      ...
      <div className="admin-deposit-page__content">
        {/* Stats */}
        <div className="admin-deposit-page__stats">
          <div className="adep-stat">
            <div className="adep-stat__icon adep-stat__icon--blue"><Clock size={20} /></div>
            <div><span className="adep-stat__val">{deposits.length}</span><span className="adep-stat__lbl">Pending</span></div>
          </div>
          <div className="adep-stat">
        ...
        {!loading && !error && deposits.length > 0 && (
          <div className="adep-table-wrap">
            <table className="adep-table">
              <thead>
                <tr>
                  <th>User</th>
                  <th>Amount</th>
              ...
              <tbody>
                {deposits.map((dep) => (
                  <tr key={dep.id} className="adep-table__row">
                    <td>
                      <div className="adep-table__user">
                        <div className="adep-table__avatar">
                          <User size={14} />
                        </div>
                        <div>
                    ...
                    <td><strong className="adep-table__amount">{fmt(dep.amount)}</strong></td>
                    <td><code className="adep-table__ref">{dep.referenceCode ?? '—'}</code></td>
                    <td className="adep-table__date">{fmtDate(dep.createdAt)}</td>
                    <td>
                      <span className="adep-badge adep-badge--pending">
                    ...
                    <td>
                      <div className="adep-table__actions">
                        <button
                          ...
                          onClick={() => openConfirm('approve', dep)}
                        >
      ...
      <ConfirmModal
        ...
        message={
          confirmModal.deposit
            ? `${confirmModal.type === 'approve' ? 'Approve' : 'Reject'} deposit of ${fmt(confirmModal.deposit.amount)} from ${confirmModal.deposit.user?.username ?? confirmModal.deposit.user?.fullName ?? 'this user'}?`
            : ''
        }
